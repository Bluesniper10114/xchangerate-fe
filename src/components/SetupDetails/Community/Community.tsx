...
class CommunityForm extends  React.Component<any, any> {
    ...
    componentWillMount() {
        ...
        if(!voteList || !voteList.votes || voteList.status.error) {
            fetchVoteList();
        }

        if(!tokenData || !tokenData.tokenCredit || tokenData.status.error) {
            fetchTokenCredit();
        }
    ...
    render() {
        const { handleSubmit, exchanges, base_coin, status, tokenData } = this.props;
        const { activeEntryIndex } = this.state;
        const loopCount = [1, 2, 3, 4];
            ...
            <div>
                ...
                <form onSubmit={handleSubmit(this.onSubmit.bind(this))}>
                    ...
                    <div className={styles.itemWrapper}>
                        ...
                        <div className={styles.itemContent}>
                            <TextField label="0x4fA60A534EEeBaBE63e1Ec3C758Ad67aF29c321c" name="wallet" border={true} disabled/>
                    ...
                    <div className={styles.communityFromTo}>
                        <span>01/06/2018</span>
                        <span className={styles.communityTo}>to</span>
                        <span>01/06/2018</span>
                    </div>
                    {
                        loopCount.map((m, index) => {
                            return (
                                <OneVote key={index}/>
                            )
                        })
